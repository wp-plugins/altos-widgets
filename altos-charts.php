...
*/
class Altos_Widgets_charts_widget /* < Register this widget class. */
	extends WP_Widget /* See: /wp-includes/widgets.php for further details. */
	{
		/*
		Constructor function.
		*/
		function Altos_Widgets_charts_widget () /* Builds the classname, id_base, description, etc. */
			{
				$widget_ops = array ("classname" => "altos-widgets-charts-widget", "description" => "Altos Charts");
				...
				if ($Altos_Widgets->is_authenticated ())
					{
						...
						echo '', $Altos_Widgets->print_checkbox_options ($this->get_field_name ("st,cid,zid"), $Altos_Widgets->get_chart_state_city_zip_codes_array (), $widget_options["st,cid,zid"]);
						echo '</div><br />';
						echo '</p>';
						/**/
						echo '<p>';
						echo '<label>';
						echo 'Statistic:<br />';
						echo '<select name="' . $this->get_field_name ("s") . '">', $Altos_Widgets->print_options ($Altos_Widgets->s[0], $widget_options["s"]), '</select>';
						echo '</label>';
						echo '</p>';
						/**/
						echo '<p>';
		...
		function update ($instance = FALSE, $old = FALSE)
			{
				...
				/**/
				foreach ((array)$widget_options["st,cid,zid"] as $st_cid_zid)
					{
						list ($widget_options["st"], $widget_options["cid"], $widget_options["zid"]) = explode (",", $st_cid_zid);
						/**/
						$charturl = $Altos_Widgets->get_state_city_zip_chart_url ($widget_options["st"], $widget_options["cid"], $widget_options["zid"], $widget_options["s"], $widget_options["sz"], $widget_options["rt"], $widget_options["ra"], $widget_options["q"], $widget_options["ts"]);
						/**/
